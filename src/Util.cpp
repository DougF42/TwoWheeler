 ...
 * if its empty
 *
 * @param tmpPtr is a pointer where we will store a pointer to a duplicate of the argument,
 *       by null that  it is truncated at the first comma or line terminator.
 *  The caller MUST free this!
 *
 * @return SYSTEM_DATA if error, WIDGET_DATA if ok.
 */
ProcessStatus Util::sanityCheck(char *arg, const char *refName, char **tmpPtr)
{
    ...
    if ((arg == nullptr) || (*arg == '\0'))
    {
        ...
        sprintf(SMACData.values, "EROR decoding %s: Missing argument! ", refName);
