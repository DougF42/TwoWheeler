...
void setup()
{
  ...
  MCUPreferences.getBytes ("RelayerMAC", RelayerMAC, MAC_SIZE);
  MCUPreferences.end      ();
...
void Serial_ProcessMessage ()
{
  ...
  else if (strncmp (Serial_Message, "NewMAC=", 7) == 0)
  {
    ...
    else
    {
      ...
      for (int i=7, j=0; j<MAC_SIZE; i+=3, j++)
      {
      ...
      MCUPreferences.begin    ("RelayerMAC", false);
      MCUPreferences.putBytes ("RelayerMAC", RelayerMAC, MAC_SIZE);
      MCUPreferences.end      ();
